...
- 不要在没读 `Qwen3_Omni_ONNX_突破看似完备的经验总结.md` 的情况下宣称“已修复/已验证/全部通过”；每次修改代码前后都要按它的检查单执行，并把新教训追加进去。

### 0.5.1 交给另一台机器的 AI 时

先读本第 0 节与 `README.md`，再读 `Qwen3_Omni_ONNX_突破看似完备的经验总结.md` 第 7 节。接管方每次汇报必须给出：三态标注（已实测/静态推断/未验证）、命令与真实退出码、本次重算的数字、已知未验证项、模块↔测试矩阵变化。禁止把“两侧一致”说成“正确”，禁止放宽判据让流程变绿。
